...
function renderDashboardTable() {
    ...
    investments.forEach(inv => {
        ...
        async function uploadCompanyPDF() {
  const company = document.getElementById("companyName").value;
  const file = document.getElementById("pdfFile").files[0];
  
  if (!company || !file) {
    alert("Please enter company name and upload a PDF file!");
    return;
  }

  // Simulate processing by calling backend Python scripts
  ...
  const data = await response.json(); // returned JSON structure with all three JSON combined

  showCompanyData(company, data);
}

    function showCompanyData(company, data) {
    const container = document.getElementById("companiesContainer");

    const card = document.createElement("div");
    card.className = "company-widget";
    card.innerHTML = `
        <h2>${company}</h2>
        <div><strong>Ratios:</strong> ${formatRatios(data.company_ratios)}</div>
        <div><strong>Financial Summary:</strong> ${formatFinancials(data.financial_summary)}</div>
        <div><strong>AI Market Analysis:</strong> ${formatAIInsights(data.ai_market_analysis)}</div>
    `;
    ...
    function formatFinancials(summary) {
    ...
    function formatAIInsights(ai) {
    return `
        <p><b>Overall Health Score:</b> ${ai.overall_health_score}</p>
        <p><b>Industry Comparison:</b> ${ai.industry_comparison}</p>
        <p><b>Key Strengths:</b> ${ai.key_strengths}</p>
        <p><b>Key Weaknesses:</b> ${ai.key_weaknesses}</p>
        <p><b>Market Position:</b> ${ai.market_position}</p>
        <p><b>Detailed Analysis:</b> ${ai.detailed_analysis}</p>
        <p><b>Benchmarks Used:</b> ${ai.benchmarks_used}</p>
        <p><b>Market Context:</b> ${ai.market_context}</p>
    `;
    }
